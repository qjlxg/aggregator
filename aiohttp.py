...
async def test_url_connectivity_async(session, url, timeout=5):
    """异步测试 URL 是否可连通"""
    try:
        async with session.head(url, timeout=timeout) as response:
            return response.status < 400  # 返回 True，如果状态码小于 400
    except aiohttp.ClientError:
        #print(f"URL {url} 连接测试失败: {e}") # 移除详细的log，否则并发时会混乱
        return False
...
async def main(base_url='https://t.me/s/jichang_list', max_pages=90):
    """主函数，控制抓取流程"""
    headers = {
    ...
    # 使用 aiohttp 进行并发测试
    logging.info("开始使用 aiohttp 并发测试 URL 连通性...")
    async with aiohttp.ClientSession() as session:
        tasks = [test_url_connectivity_async(session, url) for url in all_urls]
        results = await asyncio.gather(*tasks)  # 并发执行所有任务

    valid_urls = [url for url, result in zip(all_urls, results) if result]
...
if __name__ == '__main__':
    ...
    asyncio.run(main(start_url, max_pages_to_crawl)) # 使用 asyncio.run 来运行 main 函数
