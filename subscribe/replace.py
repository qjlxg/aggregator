...
def main(urls):
    ...
    for i in range(0, len(all_proxies), chunk_size):
        ...
        output_filename = now.strftime("%Y-%m-%d-%H%M%S") + f"_{i // chunk_size + 1}.yaml" # Add chunk number to filename
        
        output_dir = os.path.join('sub', year_month_dir)
        os.makedirs(output_dir, exist_ok=True)
        
        output_path = os.path.join(output_dir, output_filename)
        
        yaml_content = generate_yaml(chunk_proxies)
