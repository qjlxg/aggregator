...
def search_github(page: int, cookie: str, searchtype: str, sortedby: str) -> str:
    ...
    url = f"https://bgithub.xyz/search?o=desc&p={page}&q={query}&s={sortedby}&type={searchtype}"
    headers = {
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
        "Referer": "https://bgithub.xyz",
        "User-Agent": utils.USER_AGENT,
...
def search_github_issues(page: int, cookie: str) -> list[str]:
    ...
    try:
        ...
        links = [f"https://bgithub.xyz{x}" for x in links]
        return links
    except:
        return []
    ...
    try:
        ...
        for uri in uris:
            if not intercept(text=uri, excludes=excludes):
                links.add(f"https://bgithub.xyz{uri}")
...
def collect_airport(
    ...
    def crawl_jctj(convert: bool = False) -> dict:
        url = "https://raw.bgithub.xyz/hwanz/SSR-V2ray-Trojan-vpn/main/README.md"
        content = utils.http_get(url=url)
