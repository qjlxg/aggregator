...
import clash
import subconverter
from playwright.sync_api import sync_playwright, TimeoutError # 导入 playwright
...
class SubscriptionManager:
    def __init__(self, bin_name: str, num_threads: int, display: bool, repo_files: dict):
        ...
        self.registered_domains = set() #  新增：存储已注册的域名
    ...
    def assign(self, domains_file: str = "", overwrite: bool = False, pages: int = sys.maxsize, rigid: bool = True, chuck: bool = False, subscribes_file: str = "", refresh: bool = False, customize_link: str = "") -> (List[TaskConfig], dict):
        ...
        for domain, param in domains.items():
            ...
            if domain not in task_set and domain not in self.registered_domains: # 增加判断
                # 尝试注册
                registered = self.register_account(domain, param.get("coupon", ""), param.get("invite_code", ""))
                if registered:
                    tasks.append(
                        TaskConfig(
                            name=name,
                    ...
                    task_set.add(domain)
                    self.registered_domains.add(domain) #  加入到已注册集合
                else:
                    logger.warning(f"域名 {domain} 注册失败")
        return tasks, domains

    def register_account(self, domain: str, coupon: str, invite_code: str) -> bool:
        """
        尝试自动注册机场账号。
        ...
        try:
            with sync_playwright() as p:
                ...
                # 1. 构造完整的注册 URL (这里需要根据实际网站的注册流程来确定)
                register_url = urljoin(domain, "/auth/register")  # 假设注册页面是 /auth/register
                page.goto(register_url, timeout=60000)  # 增加超时时间
                # 2. 填写注册表单 (你需要根据实际网站的 HTML 结构来定位元素)
                page.fill("input[name='email']", f"testuser{random.randint(10000, 99999)}@example.com")
                page.fill("input[name='password']", "password123")
                page.fill("input[name='repassword']", "password123")
                if coupon:
                    page.fill("input[name='coupon']", coupon)
                if invite_code:
                    page.fill("input[name='invite_code']", invite_code)
                # 3. 提交注册表单
                page.click("button[type='submit']")
                # 4. 等待注册结果 (这里需要根据实际网站的注册成功提示来判断)
                try:
                    page.wait_for_selector(".alert-success", timeout=10000)  # 等待成功提示
                    logger.info(f"域名 {domain} 注册成功")
                    browser.close()
                    return True
                except TimeoutError:
                    logger.warning(f"域名 {domain} 注册失败，未找到成功提示")
                    browser.close()
                    return False

        except Exception as e:
            logger.error(f"注册域名 {domain} 发生异常: {e}")
            return False
