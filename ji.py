...
def crawl_single_source(start_url, headers, max_pages):
    """抓取单个来源的 URL"""
    all_urls = set()
    ...
    while current_url and page_count < max_pages:
        logging.info(f"正在抓取页面: {current_url} (第 {page_count + 1}/{max_pages} 页，来源: {start_url})")
...
def main(start_urls, max_pages=10):
    """主函数，控制多个来源的抓取流程"""
    headers = {
    ...
    for start_url in start_urls:
        logging.info(f"开始处理来源: {start_url}")
        source_urls = crawl_single_source(start_url, headers, max_pages)
        all_urls.update(source_urls)
        logging.info(f"完成来源: {start_url}，找到 {len(source_urls)} 个 URL")
        time.sleep(random.uniform(60, 90)) # 每个来源抓取完毕后增加延迟
