...
def get_urls_from_html(html):
    ...
    for target in targets:
        ...
        # 优化: 确保只匹配以 http 或 https 开头的完整 URL
...
def worker(url_queue, valid_urls, failed_urls, lock, pbar): # 增加 pbar 参数
    while True:
        url = url_queue.get()
        if url is None:
...
def main(start_urls):
    ...
    try:
        ...
        logging.error(f"GitHub API authentication failed with provided token: {e}. Exiting.")
        sys.exit(1)
    ...
    # 爬取完成后，获取队列中的总任务数，并启动进度条
    total_urls = url_queue.qsize()
    logging.info(f"Crawling completed. Found a total of {total_urls} URLs to process.")
    
    # 仅当有 URL 需要处理时才显示进度条
    if total_urls > 0:
        pbar = tqdm(total=total_urls, desc="Checking URLs")
    else:
        pbar = None
    ...
    for _ in range(NUM_WORKING_THREADS):
        t = threading.Thread(target=worker, args=(url_queue, valid_urls, failed_urls, lock, pbar))
        t.daemon = True
        t.start()
        threads.append(t)
    ...
    logging.info("All URL checks completed. Sending stop signals to workers.")
    ...
    logging.info(f"Number of failed URLs: {len(failed_urls)}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 优化: 在读取配置时，过滤掉注释和空行
        start_urls_list = [url.strip() for url in config_content.split('\n') if url.strip() and not url.strip().startswith('#')]
